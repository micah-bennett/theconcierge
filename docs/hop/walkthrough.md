...
| Page | What it's for |
|---|---|
| **Dashboard** | Your active request (if any), 6 quick-request tiles (Ride, Meals, Errands, Wellness, Family Care, Other), and upcoming Google Calendar events once connected. |
...
| **Settings** | Your name, phone number (so your concierge can reach you), theme (light/dark), and a link to reset your password. |
...
| **Concierges** *(ConciergeHub)* | Create concierge accounts (they get an emailed invite to set a password, or you hand them a temporary one) and enable/disable them. |
| **Users** | Every HOP member — contact info, integration status, enable/disable an account, and (ConciergeHub) a "Message" shortcut into a direct conversation. |
| **Requests** | Assign a request to any admin or concierge, move it through the workflow, log dispatch notes, and see the requester's contact info and the assignee's rating right on the card. |
...
```js
localStorage.removeItem('hop-tour-member')          // or hop-tour-concierge,
                                                       // hop-tour-admin, hop-tour-conciergehub-admin
```
